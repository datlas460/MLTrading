...
def fig2(win_prob):
    ...
    upper = meanA + stdA
    lower = meanA - stdA
    plt.plot(meanA, label='Mean')
    plt.plot(upper, label='Upper Std.Dev')
    plt.plot(lower, label='Lower Std.Dev')
    #plt.plot(a)
    plt.xlabel('Tries')
    plt.ylabel('Winnings')
    plt.legend(loc='lower left')
...
def fig3(win_prob):
    ...
    medA = np.median(a, axis=0)
    stdA = np.std(a, axis=0)
    upper = medA + stdA
    lower = medA - stdA
    plt.plot(medA, label='Median')
    plt.plot(upper, label='Upper Std.Dev')
    plt.plot(lower, label='Lower Std.Dev')
    #a = pd.concat([a,meanA] , ignore_index=True)
    # plotting.set_xlabel('Tries')
